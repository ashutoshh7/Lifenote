...
/// <summary>
/// Moved from Lifenote.Data/Repositories/NoteRepository.cs.
/// </summary>
public class NoteRepository : INoteRepository
{
    ...
    public async Task<IEnumerable<Note>> GetAllAsync(int userId) =>
        await _db.Notes.Where(n => n.UserId == userId && !n.IsArchived)
            .AsNoTracking().OrderByDescending(n => n.IsPinned).ThenByDescending(n => n.UpdatedAt).ToListAsync();

    public async Task<IEnumerable<Note>> GetPinnedAsync(int userId) =>
        await _db.Notes.Where(n => n.UserId == userId && n.IsPinned && !n.IsArchived)
            .AsNoTracking().ToListAsync();

    public async Task<IEnumerable<Note>> SearchAsync(int userId, string searchTerm) =>
        await _db.Notes.Where(n => n.UserId == userId &&
    ...
    public async Task<IEnumerable<Note>> GetByCategoryAsync(int userId, string category) =>
        await _db.Notes.Where(n => n.UserId == userId && n.Category == category)
            .AsNoTracking().ToListAsync();
